...
public static class SmartNavigation
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var segmentTasks = new Dictionary<PathSegment, Task<float?>>();

        Task<float?> GetDistance(Vector3 start, Vector3 end, bool allowKnownTransit)
        {
            if (allowKnownTransit && TryCalculateKnownTransitDistance(eventData, start, end, out var knownDistance))
            {
                return Task.FromResult<float?>(knownDistance);
            }

            var segment = new PathSegment(start, end);
            if (!segmentTasks.TryGetValue(segment, out var task))
            {
                task = MeasurePathAsync(
                    start,
                    end,
                    pathfind,
                    timeout,
                    failure,
                    cancellationToken);
                segmentTasks[segment] = task;
            }

            return task;
        }

        var directTask = GetDistance(playerPosition, destination, true);
        var baseTask = GetDistance(baseCamp.Destination, destination, true);
        var destinationTasks = destinationShards.ToDictionary(
            shard => shard.Aethernet,
            shard => GetDistance(shard.Destination, destination, true));
        var sourceTasks = sourceShards.ToDictionary(
            shard => shard.Aethernet,
            shard => GetDistance(playerPosition, shard.Position, false));

        await Task.WhenAll(segmentTasks.Values).ConfigureAwait(false);
        ...
        AddCandidateIfReachable(
            ...
            Aethernet.Unknown,
            baseCamp.Aethernet,
            AddCost(await baseTask.ConfigureAwait(false), returnCost));

        foreach (var destinationShard in destinationShards)
        {
            var destinationDistance = await destinationTasks[destinationShard.Aethernet].ConfigureAwait(false);
            ...
            {
                var sourceDistance = await sourceTasks[sourceShard.Aethernet].ConfigureAwait(false);
                AddCandidateIfReachable(
                    candidates,
